...
class PluginEncryptionManager:
    ...
    def _get_or_generate_key(self) -> bytes:
        ...
        key_env = (
            settings.PLUGIN_ENCRYPTION_KEY
            if hasattr(settings, "PLUGIN_ENCRYPTION_KEY")
            else None
        )
        ...
        for field_name, field_schema in properties.items():
            ...
            if field_schema.get("type") == "object":
                ...
                sensitive_fields.extend(
                    [f"{field_name}.{nested}" for nested in nested_sensitive]
                )
...
class PluginSchemaManager:
    ...
    async def get_plugin_schema(
        self, plugin_id: Union[str, uuid.UUID], db: AsyncSession
        ...
        if not schema:
            self.logger.warning(
                f"No config_schema in manifest for plugin {plugin.slug}"
            )
            return None
            ...
            try:
                ...
                if "." in key:
                    ...
                    if isinstance(decrypted_config[parent_key], dict):
                        ...
                        self.logger.warning(
                            f"Cannot set nested field {key} - parent is not dict"
                        )
                else:
                    # Top-level field
...
class PluginConfigurationManager:
    ...
    async def save_plugin_configuration(
        ...
        if existing_config:
            ...
            try:
                ...
                existing_data = await self.get_plugin_configuration(
                    plugin_id, user_id, db, decrypt_sensitive=True
                )
                ...
                if existing_data:
                    ...
                    if schema:
                        sensitive_fields = (
                            self.schema_manager.encryption.identify_sensitive_fields(
                                schema
                            )
                        )
                        ...
                        for field in sensitive_fields:
                            if (
                                not validation_config.get(field)
                                or str(validation_config.get(field)).strip() == ""
                            ):
                                if existing_data.get(field):
                                    validation_config[field] = existing_data[field]

                    # Validate with complete config
                    is_valid, errors = await self.schema_manager.validate_configuration(
                        plugin_id, validation_config, db
                    )
                    if is_valid:
                ...
                self.logger.warning(
                    f"Using relaxed validation due to decryption error: {e}"
                )
                ...
                if schema:
                    ...
                    sensitive_fields = (
                        self.schema_manager.encryption.identify_sensitive_fields(schema)
                    )
                    ...
                    for field in sensitive_fields:
                        if field in relaxed_schema.get("properties", {}):
                            ...
                            if (
                                not config_data.get(field)
                                or str(config_data.get(field)).strip() == ""
                            ):
                                ...
                                # Make it optional
                                if (
                                    "required" in relaxed_schema
                                    and field in relaxed_schema["required"]
                                ):
                                    relaxed_schema["required"] = [
                                        r
                                        for r in relaxed_schema["required"]
                                        if r != field
                                    ]
        ...
        if existing_config and existing_config.encrypted_data:
            ...
            if fields_to_encrypt:
                for field, value in fields_to_encrypt.items():
                    new_encrypted[field] = self.schema_manager.encryption.encrypt_value(
                        str(value)
                    )
            ...
            (
                non_sensitive,
                encrypted_sensitive,
            ) = await self.schema_manager.process_configuration_fields(
    ...
    async def get_plugin_configuration(
        ...
        if decrypt_sensitive and config.encrypted_data:
            try:
                ...
                decrypted_config = self.schema_manager.decrypt_configuration(
    ...
    async def get_resolved_configuration(
        ...
        user_config = await self.get_plugin_configuration(
            plugin_id, user_id, db, decrypt_sensitive=True
        )
