...
class TestLLMEndpoints:
    ...
    async def test_embeddings_success(self, client: AsyncClient):
        ...
        with patch(
            ...
            response = await client.post(
                ...
                json={"model": "privatemode-embeddings", "input": "Hello world"},
                headers={"Authorization": "Bearer test-api-key"},
            )
    ...
    async def test_model_validation(self, client: AsyncClient):
        ...
        from app.services.llm.exceptions import ValidationError

        with patch(
            "app.services.llm.service.llm_service.create_chat_completion"
        ) as mock_chat:
            mock_chat.side_effect = ValidationError("Invalid model")
    ...
    async def test_tee_model_detection(self, client: AsyncClient):
        ...
        tee_models = [
            model for model in data["data"] if "tee" in model.get("capabilities", [])
        ]
    ...
    async def test_provider_health_monitoring(self, client: AsyncClient):
        ...
        with patch(
            "app.services.llm.service.llm_service.get_health_summary"
        ) as mock_health_call:
            mock_health_call.return_value = mock_health

            response = await client.get(
                "/api/v1/llm/health", headers={"Authorization": "Bearer test-api-key"}
            )
    ...
    async def test_streaming_support(self, client: AsyncClient):
        ...
        data = response.json()
        streaming_models = [
            model for model in data["data"] if model.get("supports_streaming")
        ]
