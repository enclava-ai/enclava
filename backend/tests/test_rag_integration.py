...
class RAGIntegrationTest:
    ...
    async def test_content_module_integration(self):
        ...
        assert (
            "content" in modules_by_name
        ), "Content module not found in loaded modules"
        content_module = modules_by_name["content"]
        assert content_module.get(
            "initialized", False
        ), "Content module should be initialized"
    ...
    async def test_document_upload_processing(self):
        ...
        for doc in test_documents:
            ...
            files = {
                "file": (
                    doc["name"],
                    BytesIO(doc["content"].encode()),
                    doc["content_type"],
                )
            }
            data = {"collection_id": self.test_collection_id}
            ...
            print(
                f"    ✓ {doc['name']} uploaded successfully (ID: {uploaded_doc['id']})"
            )
    ...
    async def test_document_browsing_search(self):
        ...
        test_docs_found = [
            doc for doc in all_documents if doc["id"] in self.test_document_ids
        ]
        assert (
            len(test_docs_found) >= 3
        ), f"Expected at least 3 test documents, found {len(test_docs_found)}"

        print(
            f"✓ Found {len(all_documents)} total documents, {len(test_docs_found)} are our test documents"
        )
        ...
        print(
            f"✓ Collection filtering works: {len(filtered_documents)} documents in test collection"
        )
    ...
    async def test_document_deletion(self):
        ...
        assert (
            doc_to_delete not in deleted_doc_ids
        ), "Deleted document should not appear in listing"
    ...
    async def test_performance_metrics(self):
        ...
        endpoints = ["/api/rag/collections", "/api/rag/documents"]
    ...
    async def run_all_tests(self):
        ...
        try:
            ...
            print(
                f"✓ Document upload/processing tested with {len(uploaded_docs)} documents"
            )
    ...
    async with httpx.AsyncClient(timeout=10.0) as client:
        try:
            ...
            if response.status_code == 200:
                ...
                print(
                    f"✅ RAG Collections API responding: {collection_count} collections"
                )
            else:
            ...
            if response.status_code == 200:
                ...
                if "rag" in modules:
                    ...
                    print(
                        f"   {rag_status} RAG module: v{modules['rag'].get('version', 'unknown')}"
                    )

                if "content" in modules:
                    content_status = (
                        "🟢" if modules["content"].get("initialized") else "🟡"
                    )
                    print(
                        f"   {content_status} Content module: v{modules['content'].get('version', 'unknown')}"
                    )
