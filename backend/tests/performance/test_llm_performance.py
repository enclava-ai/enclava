...
class TestLLMPerformance:
    ...
    async def test_chat_completion_latency(self, client: AsyncClient):
        ...
        with patch(
            ...
            warmup_response = await client.post(
                ...
                json={
                    "model": "privatemode-llama-3-70b",
                    "messages": [{"role": "user", "content": "Performance warmup"}],
                },
    ...
    async def test_concurrent_throughput(self, client: AsyncClient):
        ...
        mock_response = ChatCompletionResponse(
            ...
            choices=[
                ChatChoice(
                    index=0,
                    message=ChatMessage(
                        role="assistant", content="Throughput test response."
                    ),
                    finish_reason="stop",
                )
            ],
        ...
        with patch(
            ...
            warmup_tasks = [
                client.post(
                    ...
                    json={
                        "model": "privatemode-llama-3-70b",
                        "messages": [
                            {"role": "user", "content": f"Concurrent warmup {i}"}
                        ],
                    },
                    headers={"Authorization": "Bearer test-api-key"},
                )
            ...
            warmup_responses = await asyncio.gather(*warmup_tasks)
    ...
    async def test_embedding_performance(self, client: AsyncClient):
        ...
        with patch(
            ...
            test_texts = [
                ...
                "Very long text that contains many words and sentences. "
            ...
            for text in test_texts:
                ...
                response = await client.post(
                    ...
                    json={"model": "privatemode-embeddings", "input": text},
                    headers={"Authorization": "Bearer test-api-key"},
                )
    ...
    async def test_memory_efficiency(self, client: AsyncClient):
        ...
        mock_response = ChatCompletionResponse(
            ...
            choices=[
                ChatChoice(
                    index=0,
                    message=ChatMessage(role="assistant", content=large_content),
                    finish_reason="stop",
                )
            ],
            usage=Usage(prompt_tokens=50, completion_tokens=500, total_tokens=550),
        )
        ...
        mock_response = ChatCompletionResponse(
            ...
            choices=[
                ChatChoice(
                    index=0,
                    message=ChatMessage(
                        role="assistant", content="Secure response with analysis."
                    ),
                    finish_reason="stop",
                )
            ],
        ...
        with patch(
            ...
            for i in range(10):
                ...
                response = await client.post(
                    ...
                    json={
                        "model": "privatemode-llama-3-70b",
                        "messages": [{"role": "user", "content": f"Security test {i}"}],
                    },
                    headers={"Authorization": "Bearer test-api-key"},
                )
