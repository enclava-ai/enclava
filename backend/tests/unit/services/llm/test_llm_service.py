...
class TestLLMService:
    ...
    async def test_chat_completion_success(
        ...
        with patch.object(
            llm_service, "_call_provider", new_callable=AsyncMock
        ) as mock_call:
    ...
    async def test_multiple_messages_handling(
        self, llm_service, mock_provider_response
    ):
    ...
    async def test_malformed_request_validation(self, llm_service):
        ...
        with pytest.raises((ValueError, Exception)):
            request = ChatRequest(
                messages=[], model="gpt-3.5-turbo", user_id="test-user"
            )
            await llm_service.chat_completion(request)
    ...
    async def test_content_filtering_input(self, llm_service):
        ...
        with patch.object(
            llm_service, "security_service", create=True
        ) as mock_security:
            mock_security.analyze_request.return_value = {
                "risk_score": 0.9,
                "blocked": True,
            }
    ...
    async def test_content_filtering_output(self, llm_service, sample_chat_request):
        ...
        with patch.object(
            ...
            with patch.object(
                ...
                mock_security.analyze_response.return_value = {
                    "risk_score": 0.8,
                    "blocked": True,
                }
    ...
    async def test_response_time_logging(self, llm_service, sample_chat_request):
        ...
        with patch.object(
            ...
            with patch.object(
                llm_service, "metrics_service", create=True
            ) as mock_metrics:
                await llm_service.chat_completion(sample_chat_request)

                # Verify metrics were recorded
                assert mock_metrics.record_request.called or hasattr(
                    mock_metrics, "record_request"
                )
    ...
    async def test_concurrent_requests_handling(self, llm_service, sample_chat_request):
        ...
        with patch.object(
            ...
            tasks = [llm_service.chat_completion(sample_chat_request) for _ in range(5)]
    ...
    @pytest.mark.asyncio
    async def test_network_interruption_handling(
        self, llm_service, sample_chat_request
    ):
        ...
        with patch.object(
            ...
            assert any(
                ...
                for keyword in ["network", "connection", "unavailable"]
            )
        ...
        with patch.object(llm_service, "budget_service", create=True) as mock_budget:
            ...
            with patch.object(
                ...
                response = await llm_service.chat_completion(
                    sample_chat_request, user_id=mock_user_id
                )
            ...
            with patch.object(
                ...
                mock_call.return_value = {
                    "choices": [
                        {
                            "message": {
                                "content": "Based on the context, machine learning is..."
                            }
                        }
                    ],
