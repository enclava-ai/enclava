...
class TestBudgetEnforcementExtended:
    ...
    @pytest.mark.asyncio
    async def test_usage_tracking_daily_aggregation(
        self, budget_service, sample_budget
    ):
        """Test daily usage aggregation and tracking"""
        with patch.object(
            budget_service, "db_session", return_value=Mock()
        ) as mock_session:
            # Mock budget lookup
            ...
            daily_total = await budget_service.get_daily_usage(
                api_key_id=1, date=datetime.now().date()
            )
    ...
    @pytest.mark.asyncio
    async def test_usage_tracking_weekly_aggregation(
        self, budget_service, sample_budget
    ):
        ...
        with patch.object(
            ...
            for usage in weekly_usages:
                with patch("datetime.datetime") as mock_datetime:
                    ...
                    await budget_service.track_usage(
                        api_key_id=1, tokens=100, cost=usage["cost"], model="gpt-4"
                    )
    ...
    async def test_usage_tracking_monthly_rollover(self, budget_service, sample_budget):
        ...
        with patch.object(
            ...
            with patch("datetime.datetime") as mock_datetime:
                ...
                await budget_service.track_usage(
                    api_key_id=1, tokens=1000, cost=Decimal("20.00"), model="gpt-4"
                )
            ...
            with patch("datetime.datetime") as mock_datetime:
                ...
                await budget_service.track_usage(
                    api_key_id=1, tokens=500, cost=Decimal("10.00"), model="gpt-4"
                )
            ...
            previous_usage = await budget_service.get_month_usage(
                api_key_id=1, year=previous_month.year, month=previous_month.month
            )
    ...
    async def test_budget_isolation_between_users(self, budget_service):
        ...
        with patch.object(
            ...
            await budget_service.track_usage(
                api_key_id=1, tokens=500, cost=Decimal("10.00"), model="gpt-3.5-turbo"
            )

            # Track usage for user 2
            await budget_service.track_usage(
                api_key_id=2, tokens=1000, cost=Decimal("25.00"), model="gpt-4"
            )
    ...
    async def test_budget_check_isolation(self, budget_service):
        ...
        with patch.object(
            ...
            can_proceed_1 = await budget_service.check_budget(
                api_key_id=1, estimated_cost=Decimal("10.00")
            )
            assert can_proceed_1 is True

            # User 2 should be blocked
            can_proceed_2 = await budget_service.check_budget(
                api_key_id=2, estimated_cost=Decimal("10.00")
            )
    ...
    async def test_expired_budget_handling(self, budget_service, sample_budget):
        ...
        with patch.object(
            ...
            can_proceed = await budget_service.check_budget(
                api_key_id=1, estimated_cost=Decimal("1.00")
            )

            assert can_proceed is False

    @pytest.mark.asyncio
    async def test_budget_auto_deactivation_on_expiry(
        self, budget_service, sample_budget
    ):
    ...
    async def test_token_based_cost_calculation(self, budget_service):
        ...
        test_cases = [
            ...
            (
                ...
                500,
                Decimal("0.0020"),
            ),  # $0.001/1K input, $0.002/1K output
            (
                "gpt-4",
                1000,
                500,
                Decimal("0.0450"),
            ),  # $0.030/1K input, $0.060/1K output
    ...
    async def test_bulk_discount_calculation(self, budget_service):
        ...
        with patch.object(budget_service, "_get_user_pricing_tier") as mock_tier:
            ...
            base_cost = await budget_service.calculate_cost(
                model="gpt-3.5-turbo", input_tokens=high_volume_tokens, output_tokens=0
            )

            discounted_cost = await budget_service.apply_volume_discount(
                cost=base_cost, monthly_volume=high_volume_tokens
            )

            # Should apply enterprise discount
            expected_discount = base_cost * Decimal("0.20")
            assert abs(discounted_cost - (base_cost - expected_discount)) < Decimal(
                "0.01"
            )
    ...
    async def test_prorated_budget_mid_month(self, budget_service):
        ...
        with patch("datetime.datetime") as mock_datetime:
            ...
            prorated_limit = await budget_service.calculate_prorated_limit(
                monthly_limit=monthly_limit, creation_date=creation_date, reset_day=1
            )
    ...
    async def test_budget_overage_tracking(self, budget_service, sample_budget):
        ...
        with patch.object(
            ...
            await budget_service.track_usage(
                api_key_id=1, tokens=2500, cost=overage_cost, model="gpt-4"
            )
    ...
    async def test_budget_soft_vs_hard_limits(self, budget_service, sample_budget):
        ...
        with patch.object(
            ...
            can_proceed = await budget_service.check_budget(
                api_key_id=1, estimated_cost=Decimal("5.00")
            )
            assert can_proceed is True
