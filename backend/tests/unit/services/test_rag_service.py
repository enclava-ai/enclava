...
class TestRAGService:
    ...
    def mock_qdrant_client(self):
        ...
        mock_client.search.return_value = [
            Mock(
                id="doc1",
                payload={"content": "Sample content 1", "metadata": {"score": 0.95}},
            ),
            Mock(
                id="doc2",
                payload={"content": "Sample content 2", "metadata": {"score": 0.87}},
            ),
        ]
        return mock_client
        ...
        with patch.object(rag_service, "qdrant_client") as mock_qdrant:
            ...
            with patch.object(rag_service, "db_session") as mock_db:
                ...
                mock_qdrant.delete_collection.assert_called_once_with(
                    sample_collection.qdrant_collection_name
                )
    ...
    async def test_search_success(
        self, rag_service, sample_collection, mock_qdrant_client
    ):
            ...
            with patch.object(rag_service, "embedding_service") as mock_embeddings:
                ...
                with patch.object(rag_service, "db_session") as mock_db:
                    ...
                    results = await rag_service.search(
                        collection_id=1, query=query, top_k=5
                    )
        ...
        with patch.object(rag_service, "qdrant_client", mock_qdrant_client):
            with patch.object(rag_service, "embedding_service") as mock_embeddings:
                ...
                with patch.object(rag_service, "db_session") as mock_db:
                    ...
                    results = await rag_service.search(
                        collection_id=1, query=query, filters=filters, top_k=3
                    )
                    ...
                    assert (
                        "filter" in search_call[1] or "query_filter" in search_call[1]
                    )
    ...
    async def test_search_result_ranking(self, rag_service, sample_collection):
        ...
        mock_results = [
            Mock(
                id="doc1",
                payload={"content": "Low relevance", "metadata": {}},
                score=0.6,
            ),
            Mock(
                id="doc2",
                payload={"content": "High relevance", "metadata": {}},
                score=0.9,
            ),
            Mock(
                id="doc3",
                payload={"content": "Medium relevance", "metadata": {}},
                score=0.75,
            ),
        ]
        ...
        with patch.object(rag_service, "qdrant_client") as mock_qdrant:
            ...
            with patch.object(rag_service, "embedding_service") as mock_embeddings:
                ...
                with patch.object(rag_service, "db_session") as mock_db:
                    ...
                    results = await rag_service.search(
                        collection_id=1, query="test", top_k=5
                    )
    ...
    @pytest.mark.asyncio
    async def test_search_score_threshold_filtering(
        self, rag_service, sample_collection
    ):
        """Test filtering results by minimum score threshold"""
        mock_results = [
            Mock(
                id="doc1", payload={"content": "High score", "metadata": {}}, score=0.9
            ),
            Mock(
                id="doc2", payload={"content": "Low score", "metadata": {}}, score=0.3
            ),
        ]
        ...
        with patch.object(rag_service, "qdrant_client") as mock_qdrant:
            ...
            with patch.object(rag_service, "embedding_service") as mock_embeddings:
                ...
                with patch.object(rag_service, "db_session") as mock_db:
                    ...
                    results = await rag_service.search(
                        collection_id=1, query="test", min_score=0.5
                    )
    ...
    async def test_empty_query_handling(self, rag_service, sample_collection):
        ...
        for query in empty_queries:
            ...
            assert (
                ...
                and "empty" in str(exc_info.value).lower()
            )
    ...
    async def test_end_to_end_document_workflow(self, rag_service):
        ...
        with patch.object(rag_service, "qdrant_client") as mock_qdrant:
            ...
            mock_qdrant.search.return_value = [
                Mock(
                    id="doc1",
                    payload={"content": "Test document content", "metadata": {}},
                    score=0.9,
                )
            ]
                ...
                with patch.object(rag_service, "embedding_service") as mock_embeddings:
                    ...
                    with patch.object(rag_service, "db_session") as mock_db:
                        ...
                        mock_db.query.return_value.filter.return_value.first.side_effect = [
                            ...
                            Mock(
                                id=1, qdrant_collection_name="e2e_test_collection"
                            ),  # Collection exists for document add
                            Mock(
                                id=1, qdrant_collection_name="e2e_test_collection"
                            ),  # Collection exists for search
                        ]
                        ...
                        results = await rag_service.search(
                            collection_id=1, query="test document"
                        )
                        assert len(results) == 1
