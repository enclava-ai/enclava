...
class TestBudgetEnforcement:
    ...
    def test_check_budget_under_limit(
        self, budget_service, sample_budget, sample_api_key
    ):
        """Test budget check when under limit."""
        with patch.object(
            budget_service, "_get_applicable_budgets", return_value=[sample_budget]
        ):
    ...
    def test_check_budget_over_limit(
        ...
        with patch.object(
            budget_service, "_get_applicable_budgets", return_value=[over_limit_budget]
        ):
            allowed, message, warnings = budget_service.check_budget_compliance(
                api_key=sample_api_key,
                model_name="gpt-3.5-turbo",
    ...
    def test_record_usage_success(self, budget_service, sample_budget, sample_api_key):
        ...
        with patch.object(
            ...
            with patch.object(budget_service.db, "commit"):
                ...
                assert (
                    len(updated_budgets) >= 0
                )  # May or may not update depending on budget periods

    def test_record_usage_multiple_budgets(
        self, budget_service, sample_budget, daily_budget, sample_api_key
    ):
        """Test usage recording with multiple applicable budgets."""
        with patch.object(
            budget_service,
            "_get_applicable_budgets",
            return_value=[sample_budget, daily_budget],
        ):
            with patch.object(budget_service.db, "commit"):
                updated_budgets = budget_service.record_usage(
                    api_key=sample_api_key,
    ...
    def test_daily_budget_period(self, budget_service, daily_budget, sample_api_key):
        ...
        with patch.object(
            budget_service, "_get_applicable_budgets", return_value=[daily_budget]
        ):
            allowed, message, warnings = budget_service.check_budget_compliance(
                api_key=sample_api_key,
                model_name="gpt-3.5-turbo",
    ...
    def test_weekly_budget_period(self, budget_service, weekly_budget, sample_api_key):
        ...
        with patch.object(
            budget_service, "_get_applicable_budgets", return_value=[weekly_budget]
        ):
            allowed, message, warnings = budget_service.check_budget_compliance(
                api_key=sample_api_key,
                model_name="gpt-3.5-turbo",
    ...
    def test_yearly_budget_period(self, budget_service, yearly_budget, sample_api_key):
        ...
        with patch.object(
            budget_service, "_get_applicable_budgets", return_value=[yearly_budget]
        ):
            allowed, message, warnings = budget_service.check_budget_compliance(
                api_key=sample_api_key,
                model_name="gpt-3.5-turbo",
        ...
        with patch.object(budget_service.db, "add"):
            with patch.object(budget_service.db, "commit"):
                budget = budget_service.create_default_user_budget(
                    user_id=1, limit_dollars=50.0, period_type="monthly"
                )
