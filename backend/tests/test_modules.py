...
try:
    from app.modules.agent.main import AgentModule
    from app.modules.rag.main import RAGModule
    from app.services.module_manager import ModuleConfig, ModuleManager
...
class TestModuleIndividual:
    ...
    @pytest.mark.asyncio
    async def test_agent_module_initialization(self):
        """Test agent module initialization and basic operations"""
        agent_module = AgentModule()

        with patch(
            "app.modules.agent.main.llm_service.initialize", new_callable=AsyncMock
        ):
            result = await agent_module.initialize()
        assert result is True
        assert agent_module.initialized is True

        permissions = agent_module.get_required_permissions()
        assert permissions

        await agent_module.cleanup()
...
class TestModuleIntegration:
    ...
    async def test_module_manager_initialization(self):
        ...
        agent_module = module_manager.get_module("agent")
    ...
    async def test_module_dependencies(self):
        ...
        module_order = module_manager.module_order
        for module_name in ["rag", "agent", "workflow", "extract"]:
            assert module_name in module_order

        assert module_order.index("rag") < module_order.index("agent")
        assert module_order.index("agent") < module_order.index("workflow")
...
class TestModulePerformance:
    ...
    async def test_module_initialization_speed(self):
        ...
        assert (
            initialization_time < 30.0
        ), f"Module initialization took {initialization_time:.2f}s"
...
class TestModuleAPI:
    ...
    async def test_modules_api_response_structure(self):
        ...
        assert {"rag", "agent", "workflow", "extract"}.issubset(
            {module_info["name"] for module_info in api_response["modules"]}
        )
...
if __name__ == "__main__":
    ...
    async def run_basic_tests():
        ...
        await test_individual.test_agent_module_initialization()
        print("✓ Agent module test passed")
