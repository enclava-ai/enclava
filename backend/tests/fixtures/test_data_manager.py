...
class TestDataManager:
    ...
    async def create_qdrant_collection(
        self, collection_name: Optional[str] = None, vector_size: int = 1536
    ) -> str:
    ...
    async def create_test_rag_collection(
        self,
        user_id: str,
        name: Optional[str] = None,
    ...
    async def create_sample_rag_documents(self) -> List[Dict[str, Any]]:
        ...
        return [
            {
                "text": "Enclava Platform is a comprehensive AI platform that provides secure LLM services. It features agent creation, RAG integration, and OpenAI-compatible API endpoints.",
            ...
            {
                "text": "To create an agent in Enclava, navigate to the Agents section and click 'Create Agent'. Configure the model, temperature, tools, and system prompt according to your needs.",
                "filename": "agent_guide.txt",
                "document_id": "doc2",
                "chunk_index": 0,
                "metadata": {"category": "tutorial"},
            },
            {
                "text": "RAG (Retrieval Augmented Generation) allows agents to use specific documents as knowledge sources. Upload documents to a collection, then link the collection to your agent for enhanced responses.",
    ...
    async def cleanup_all(self):
        ...
        for collection_name in self.created_resources["qdrant_collections"]:
            try:
                ...
                print(
                    f"Warning: Failed to delete Qdrant collection {collection_name}: {e}"
                )
