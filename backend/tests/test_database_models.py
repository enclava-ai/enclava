...
from app.models.budget import Budget, BudgetPeriod, BudgetType, UsageTracking
...
class TestBudgetModel:
    ...
    def test_budget_usage_relationship(self, test_db):
        ...
        budget_usage = (
            test_db.query(UsageTracking)
            .filter(UsageTracking.budget_id == budget.id)
            .all()
        )
...
class TestAuditLogModel:
    ...
    def test_audit_log_creation(self, test_db):
        ...
        audit_log = AuditLog(
            ...
            metadata={"login_method": "password", "session_id": "session_456"},
        )
    ...
    def test_audit_log_querying(self, test_db):
        ...
        login_logs = (
            test_db.query(AuditLog).filter(AuditLog.action == "user_login").all()
        )
        assert len(login_logs) == 2

        # Test filtering by success
        failed_logs = test_db.query(AuditLog).filter(AuditLog.success == False).all()
        assert len(failed_logs) == 1
        assert failed_logs[0].error_message == "Invalid password"

        # Test filtering by user
        user_123_logs = (
            test_db.query(AuditLog).filter(AuditLog.user_id == "user_123").all()
        )
...
class TestModuleConfigModel:
    ...
    def test_module_config_unique_name(self, test_db):
        ...
        config1 = ModuleConfig(
            name="test_module", enabled=True, config={"setting1": "value1"}
        )
        test_db.add(config1)
        test_db.commit()
    ...
    def test_module_config_json_fields(self, test_db):
        ...
        complex_config = {
            ...
            "limits": {"max_requests": 1000, "rate_limit": 100},
        }
