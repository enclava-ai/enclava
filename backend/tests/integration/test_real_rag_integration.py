...
class RealRAGIntegrationTest:
    ...
    async def create_test_collection(self) -> Dict[str, Any]:
        ...
        async with self.session.post(
            ...
            if response.status != 200:
                ...
                raise Exception(
                    f"Failed to create collection: {response.status} - {error_text}"
                )
            ...
            print(
                f"✅ Created collection: {collection['name']} (ID: {self.test_collection_id})"
            )
    ...
    async def upload_attention_pdf(self) -> Dict[str, Any]:
        ...
        async with self.session.post(
            f"{self.frontend_url}/api/rag/documents", data=form_data, headers=headers
        ) as response:
            if response.status != 200:
                error_text = await response.text()
                raise Exception(
    ...
    async def wait_for_processing(
        self, document_id: str, max_wait: int = 60
    ) -> Dict[str, Any]:
        ...
        while time.time() - start_time < max_wait:
            async with self.session.get(
                f"{self.frontend_url}/api/rag/documents/{document_id}", headers=headers
            ) as response:
                if response.status == 200:
                    data = await response.json()
                    if data.get("success"):
                        ...
                        if status in ["processed", "indexed", "error"]:
                            if status == "error":
                                ...
                                if document.get("vector_count"):
                                    print(
                                        f"   Vector count: {document['vector_count']:,}"
                                    )
    ...
    async def test_document_content(self, document_id: str) -> None:
        ...
        async with self.session.get(
            ...
            if converted_content:
                print(
                    f"✅ Document converted to markdown ({len(converted_content)} characters)"
                )
                ...
                found_terms = [
                    ...
                    if term.lower() in converted_content.lower()
                ]
                ...
                if found_terms:
                    ...
                    print(
                        "⚠️  No specific attention paper terms found (might be mock content)"
                    )

                # Show a snippet
                snippet = (
                    converted_content[:200] + "..."
                    if len(converted_content) > 200
    ...
    async def test_rag_stats(self) -> Dict[str, Any]:
        ...
        async with self.session.get(
            f"{self.frontend_url}/api/rag/stats", headers=headers
        ) as response:
            if response.status != 200:
                error_text = await response.text()
                raise Exception(
            ...
            print(
                f"   📁 Collections: {collections.get('total', 0)} total, {collections.get('active', 0)} active"
            )

            # Documents stats
            documents = stats.get("documents", {})
            print(
                f"   📄 Documents: {documents.get('total', 0)} total, {documents.get('processed', 0)} processed, {documents.get('processing', 0)} processing"
            )
        ...
        for doc_id in self.test_document_ids:
            try:
                async with self.session.delete(
                    f"{self.frontend_url}/api/rag/documents/{doc_id}", headers=headers
                ) as response:
                    if response.status == 200:
                        print(f"✅ Deleted document {doc_id}")
                    else:
                        print(
                            f"⚠️  Failed to delete document {doc_id}: {response.status}"
                        )
            ...
            try:
                async with self.session.delete(
                    ...
                    if response.status == 200:
                        ...
                        print(
                            f"⚠️  Failed to delete collection {self.test_collection_id}: {response.status}"
                        )
            except Exception as e:
    ...
    async def run_full_test(self) -> bool:
        ...
        try:
            ...
            print(
                f"✅ Stats working: {stats['documents']['total']} documents in system"
            )
...
async def main():
    ...
    for config in test_configs:
        ...
        async with RealRAGIntegrationTest(
            frontend_url=config["frontend_url"], backend_url=config["backend_url"]
        ) as test_runner:
