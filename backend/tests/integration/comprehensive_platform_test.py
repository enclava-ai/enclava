...
class PlatformTester:
        ...
        try:
            ...
            try:
                async with self.session.get(f"{self.frontend_url}") as response:
                    if response.status == 200:
                        self.log_test(
                            "Frontend Availability", True, "Frontend accessible"
                        )
                    else:
                        self.log_test(
                            "Frontend Availability", False, f"HTTP {response.status}"
                        )
    ...
    async def test_user_authentication(self):
        ...
        try:
            ...
            async with self.session.post(
                f"{self.backend_url}/api/v1/auth/register", json=register_data
            ) as response:
                if response.status == 201:
                    ...
                    self.log_test(
                        "User Registration",
                        True,
                        f"User created: {user_data.get('email')}",
                        user_data,
                    )
                else:
            ...
            async with self.session.post(
                f"{self.backend_url}/api/v1/auth/login", json=login_data
            ) as response:
                if response.status == 200:
                    ...
                    error_data = await response.json()
                    self.log_test(
                        "User Login", False, f"HTTP {response.status}: {error_data}"
                    )
                    return False
            ...
            async with self.session.get(
                ...
                if response.status == 200:
                    ...
                    self.log_test(
                        "Token Verification",
                        True,
                        f"User info retrieved: {user_info.get('email')}",
                        user_info,
                    )
                else:
                    error_data = await response.json()
    ...
    async def test_api_key_management(self):
        ...
        try:
            ...
            async with self.session.post(
                ...
                if response.status == 201:
                    ...
                    self.log_test(
                        "API Key Creation",
                        True,
                        f"Key created: {api_key_response.get('name')}",
                        {
            ...
            # List API keys
            async with self.session.get(
                f"{self.backend_url}/api/v1/api-keys", headers=headers
            ) as response:
                if response.status == 200:
    ...
    async def test_budget_system(self):
        ...
        try:
            ...
            async with self.session.post(
                f"{self.backend_url}/api/v1/budgets", json=budget_data, headers=headers
            ) as response:
                if response.status == 201:
                    ...
                    self.log_test(
                        "Budget Creation",
                        True,
                        f"Budget created: {budget_response.get('name')}",
                        {
            ...
            # Get budget status
            async with self.session.get(
                f"{self.backend_url}/api/v1/llm/budget/status", headers=headers
            ) as response:
                if response.status == 200:
    ...
    async def test_llm_integration(self):
        ...
        try:
            ...
            async with self.session.get(
                f"{self.backend_url}/api/v1/llm/models", headers=headers
            ) as response:
                if response.status == 200:
                    ...
                    error_data = await response.json()
                    self.log_test(
                        "List Models", False, f"HTTP {response.status}: {error_data}"
                    )
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    self.log_test(
                        "Chat Completion",
                        True,
                        f"Joke received ({tokens_used} tokens)",
                        {
                            "joke_preview": (
                                joke[:100] + "..." if len(joke) > 100 else joke
                            ),
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    embedding_dim = len(
                        embedding_response.get("data", [{}])[0].get("embedding", [])
                    )
                    self.log_test(
                        "Embeddings",
                        True,
                        f"Embedding generated ({embedding_dim} dimensions)",
                        {
                            ...
                            "tokens_used": embedding_response.get("usage", {}).get(
                                "total_tokens", 0
                            ),
                        },
                    )
                    ...
                    error_data = await response.json()
                    self.log_test(
                        "Embeddings", False, f"HTTP {response.status}: {error_data}"
                    )
                ...
                async with self.session.get(f"{ollama_url}/api/tags") as response:
                    if response.status == 200:
                        ...
                        self.log_test(
                            "Ollama Connection",
                            True,
                            f"Connected to Ollama ({model_count} models)",
                            {
                                "model_count": model_count,
                                "models": [
                                ...
                                async with self.session.post(
                                    ...
                                    if chat_response.status == 200:
                                        ...
                                        self.log_test(
                                            "Ollama Chat",
                                            False,
                                            f"HTTP {chat_response.status}",
                                        )
                            except asyncio.TimeoutError:
                                self.log_test(
                                    "Ollama Chat",
                                    False,
                                    "Timeout - model may be loading",
                                )
                        else:
                            self.log_test(
                                "Ollama Models", False, "No models available in Ollama"
                            )
                    else:
                        self.log_test(
                            "Ollama Connection", False, f"HTTP {response.status}"
                        )
            except Exception as e:
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    self.log_test(
                        ...
                        True,
                        f"Collection created: {collection.get('name')}",
                        {
...
"""
            ...
            async with self.session.post(
                ...
                if response.status == 200:
                    ...
                    self.log_test(
                        "RAG Document Upload",
                        True,
                        f"Document uploaded: {document.get('filename')}",
                        {
            ...
            if processing_complete:
                ...
                async with self.session.get(
                    f"{self.backend_url}/api/v1/rag/stats", headers=headers
                ) as response:
                    if response.status == 200:
                        rag_stats = await response.json()
                        stats = rag_stats.get("stats", {})
                        self.log_test(
                            "RAG Statistics", True, "RAG stats retrieved", stats
                        )
                    else:
                        error_data = await response.json()
                        self.log_test(
    ...
    async def test_module_system(self):
        ...
        try:
            # Test modules status
            async with self.session.get(
                f"{self.backend_url}/api/v1/modules/status"
            ) as response:
                if response.status == 200:
                    ...
                    self.log_test(
                        ...
                        True,
                        f"{enabled_count}/{total_count} modules enabled",
                        {
            ...
            for module_name in test_modules:
                async with self.session.get(
                    f"{self.backend_url}/api/v1/modules/{module_name}"
                ) as response:
                    if response.status == 200:
    ...
    async def cleanup_test_data(self):
        ...
        try:
                ...
                try:
                    async with self.session.delete(
                        ...
                        if response.status == 200:
                            ...
                            logger.warning(
                                f"Failed to delete document {doc_id}: HTTP {response.status}"
                            )
                except Exception as e:
                ...
                try:
                    async with self.session.delete(
                        ...
                        if response.status == 200:
                            ...
                            logger.warning(
                                f"Failed to delete collection {collection_id}: HTTP {response.status}"
                            )
                except Exception as e:
                ...
                try:
                    async with self.session.delete(
                        ...
                        if response.status == 200:
                            ...
                            logger.warning(
                                f"Failed to delete budget {budget_id}: HTTP {response.status}"
                            )
                except Exception as e:
            ...
            for api_key in self.api_keys:
                ...
                try:
                    async with self.session.delete(
                        f"{self.backend_url}/api/v1/api-keys/{key_id}", headers=headers
                    ) as response:
                        if response.status == 200:
                            logger.info(f"Deleted API key {key_id}")
                        else:
                            logger.warning(
                                f"Failed to delete API key {key_id}: HTTP {response.status}"
                            )
                except Exception as e:
