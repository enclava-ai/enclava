...
class TestLLMEndpoints:
        ...
        with patch("app.api.v1.llm.require_api_key") as mock_auth:
            ...
            with patch("app.api.v1.llm.get_cached_models") as mock_get_models:
                ...
                response = await client.get(
                    "/api/v1/llm/models", headers=api_key_header
                )
    ...
    async def test_chat_completion_success(
        self, client, api_key_header, sample_chat_request
    ):
    ...
    async def test_chat_completion_budget_exceeded(
        ...
        with patch("app.api.v1.llm.require_api_key") as mock_auth:
            ...
            with patch("app.api.v1.llm.check_budget_for_request") as mock_budget:
                ...
                data = response.json()
                assert (
                    "budget" in data["detail"].lower()
                    or "limit" in data["detail"].lower()
                )
    ...
    async def test_chat_completion_empty_messages(self, client, api_key_header):
        ...
        response = await client.post(
            "/api/v1/llm/chat/completions", json=invalid_request, headers=api_key_header
        )
    ...
    @pytest.mark.asyncio
    async def test_chat_completion_invalid_parameters(
        self, client, api_key_header, sample_chat_request
    ):
    ...
    async def test_embeddings_success(
        self, client, api_key_header, sample_embedding_request
    ):
    ...
    async def test_embeddings_empty_input(self, client, api_key_header):
        ...
        empty_request = {"model": "text-embedding-ada-002", "input": ""}

        response = await client.post(
            "/api/v1/llm/embeddings", json=empty_request, headers=api_key_header
        )
    ...
    async def test_content_filtering_integration(self, client, api_key_header):
        ...
        harmful_request = {
            "model": "gpt-3.5-turbo",
            "messages": [{"role": "user", "content": "How to make explosive devices"}],
        }
        ...
        with patch("app.api.v1.llm.require_api_key") as mock_auth:
            ...
            with patch("app.api.v1.llm.check_budget_for_request") as mock_budget:
                ...
                with patch("app.api.v1.llm.llm_service") as mock_llm:
                    ...
                    mock_llm.chat_completion.side_effect = Exception(
                    ...
                    assert (
                        ...
                        or "safety" in data["detail"].lower()
                    )
