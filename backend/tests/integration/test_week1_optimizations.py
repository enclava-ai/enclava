...
class PerformanceTest:
    ...
    async def test_models_endpoint_caching(self):
            ...
            for i in range(TEST_ITERATIONS):
                try:
                    duration = await self.time_request(
                        session,
                        "GET",
                        f"{PLATFORM_BASE_URL}/api/v1/llm/models",
                        headers={
                            "Authorization": "Bearer dummy_jwt_token"
                        },  # Will fail auth but should still test routing
                    )
                    if duration > 0:
        ...
        return {
            ...
            "overhead_ms": (
                (statistics.mean(platform_times) - statistics.mean(litellm_times))
    ...
    async def test_health_endpoints(self):
        ...
        return {
            "platform_health_avg": (
                statistics.mean(platform_health_times) if platform_health_times else 0
            ),
            "platform_health_min": (
                min(platform_health_times) if platform_health_times else 0
            ),
            "platform_health_max": (
                max(platform_health_times) if platform_health_times else 0
            ),
    ...
    async def test_concurrent_requests(self):
        ...
        async def make_concurrent_requests(session, num_concurrent=5):
            ...
            return {
                ...
                "average_individual_time": (
                    statistics.mean(successful_results) if successful_results else 0
                ),
            }
        ...
        async with aiohttp.ClientSession() as session:
            ...
            return {
                ...
                "sequential_avg_individual": (
                    statistics.mean(sequential_times) if sequential_times else 0
                ),
                "concurrent_total_ms": concurrent_result["total_time_ms"],
                "concurrent_avg_individual": concurrent_result[
                    "average_individual_time"
                ],
                "concurrency_improvement_pct": (
                    (
                        (sequential_total - concurrent_result["total_time_ms"])
                        / sequential_total
                        * 100
                    )
    ...
    async def run_all_tests(self):
        ...
        print(
            f"  LiteLLM Direct: {models_results['litellm_avg']:.2f}ms avg ({models_results['litellm_min']:.2f}-{models_results['litellm_max']:.2f}ms)"
        )
        print(
            f"  Platform API: {models_results['platform_avg']:.2f}ms avg ({models_results['platform_min']:.2f}-{models_results['platform_max']:.2f}ms)"
        )
        ...
        print(
            f"  Platform Health: {health_results['platform_health_avg']:.2f}ms avg ({health_results['platform_health_min']:.2f}-{health_results['platform_health_max']:.2f}ms)"
        )
        ...
        print(
            f"  Sequential (5 requests): {concurrent_results['sequential_total_ms']:.2f}ms total"
        )
        print(
            f"  Concurrent (5 requests): {concurrent_results['concurrent_total_ms']:.2f}ms total"
        )
        print(
            f"  Concurrency improvement: {concurrent_results['concurrency_improvement_pct']:.1f}%"
        )
...
async def main():
    ...
    concurrent_improvement = results["concurrent_requests"][
        "concurrency_improvement_pct"
    ]
