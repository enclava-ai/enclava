        ...
        try:
            ...
            expensive_chat_data = {
                ...
                "messages": [
                    {
                        "role": "user",
                        "content": "Write a very long essay about artificial intelligence, machine learning, and the future of technology. Include detailed examples and explanations.",
                    }
                ],
            ...
            async with session.post(
                ...
                if response.status == 200:
                    ...
                    error_data = await response.json()
                    print(
                        f"🛡️  Expensive request: Blocked by budget - {error_data.get('detail', 'Unknown')}"
                    )
                else:
                    error_data = await response.json()
