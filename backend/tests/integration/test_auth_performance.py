...
class AuthPerformanceTest:
    ...
    async def test_auth_endpoint_performance(self):
        ...
        headers = {
            "Authorization": "Bearer ce_dummy_key_12345678_test_performance_auth_flow"
        }
            ...
            for concurrency in concurrency_levels:
                ...
                if valid_durations:
                    results[f"concurrency_{concurrency}"] = {
                        ...
                        "requests_per_second": (len(valid_durations) / total_time)
                        * 1000,
    ...
    async def run_all_tests(self):
        ...
        for key, data in concurrent_results.items():
            ...
            print(
                f"  {concurrency} concurrent: {data['average_request_time']:.2f}ms avg, "
    ...
    def analyze_performance(self):
        ...
        cache_improvement = self.results.get("cache_warmup", {}).get(
            "improvement_pct", 0
        )
        ...
        if concurrent_data:
            single_rps = concurrent_data.get("concurrency_1", {}).get(
                "requests_per_second", 0
            )
            high_rps = concurrent_data.get("concurrency_20", {}).get(
                "requests_per_second", 0
            )
        ...
        if cache_improvement > 30 and auth_avg < 100:
            ...
            print(
                f"  ⚠️  The optimization may need further tuning or Redis isn't available"
            )
...
async def main():
    ...
    print(
        "We're measuring the time it takes to process the authentication, not success\n"
    )
