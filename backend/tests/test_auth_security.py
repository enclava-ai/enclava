...
from app.services.api_key_auth import get_api_key_info, validate_api_key
...
class TestAPIKeySecurity:
    ...
    async def test_api_key_validation(self):
        ...
        with patch(
            "app.services.api_key_auth.get_api_key_from_db",
    ...
    async def test_expired_api_key(self):
        ...
        with patch(
            "app.services.api_key_auth.get_api_key_from_db",
            return_value=expired_api_key_data,
        ):
            result = await validate_api_key("test_api_key")
            assert result is False
    ...
    async def test_disabled_api_key(self):
        ...
        with patch(
            "app.services.api_key_auth.get_api_key_from_db",
            return_value=disabled_api_key_data,
        ):
            result = await validate_api_key("test_api_key")
            assert result is False
...
class TestPermissionSystem:
    ...
    def test_permission_checking(self):
        ...
        assert self.permission_registry.check_permission(
            user_permissions, "modules:rag:search"
        )
        ...
        assert self.permission_registry.check_permission(
            user_permissions, "admin:settings"
        )

        # Non-matching should fail
        assert not self.permission_registry.check_permission(
            user_permissions, "modules:rag:delete"
        )
        assert not self.permission_registry.check_permission(
            user_permissions, "llm:embeddings"
        )
    ...
    def test_module_permission_registration(self):
        ...
        user_perms = [
            "modules:test_module:documents:read",
            "modules:test_module:settings:*",
        ]

        assert self.permission_registry.check_permission(
            user_perms, "modules:test_module:documents:read"
        )
        assert self.permission_registry.check_permission(
            user_perms, "modules:test_module:settings:configure"
        )
        assert not self.permission_registry.check_permission(
            user_perms, "modules:test_module:documents:write"
        )
...
class TestModuleInterceptors:
    ...
    async def test_validation_interceptor(self):
        ...
        dangerous_request = {
            ...
            "data": {"code": "eval('malicious code')", "safe": "This is safe content"},
        }

        sanitized_request, context = await interceptor.pre_process(
            dangerous_request, {}
        )
    ...
    async def test_security_interceptor_suspicious_patterns(self):
        ...
        result_request, result_context = await interceptor.pre_process(
            suspicious_request, {}
        )
...
class TestEndToEndSecurity:
    ...
    async def test_full_interceptor_chain(self):
        ...
        request = {"action": "execute", "data": "test data"}
        context = {
            "user_id": "user_123",
            "api_key_id": "key_123",
