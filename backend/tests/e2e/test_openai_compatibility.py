...
from tests.clients.openai_test_client import (
    AsyncOpenAITestClient,
    OpenAITestClient,
...
class TestOpenAICompatibility:
    ...
    def openai_client(self, test_api_key):
        ...
        return OpenAITestClient(base_url=self.BASE_URL, api_key=test_api_key)

    @pytest.fixture
    def async_openai_client(self, test_api_key):
        """Async OpenAI client for performance testing"""
        return AsyncOpenAITestClient(base_url=self.BASE_URL, api_key=test_api_key)
    ...
    def test_completions_legacy(self, openai_client):
        ...
        try:
            response = openai_client.create_completion(
                model="test-model", prompt="Say hello", max_tokens=50
            )
    ...
    async def test_concurrent_requests(self, async_openai_client):
        ...
        assert (
            success_rate >= 0.5
        ), f"Low success rate for concurrent requests: {success_rate}"
        ...
        if response_times:
            ...
            ), f"High average response time: {avg_response_time}s"
    ...
    async def test_streaming_performance(self, async_openai_client):
        ...
        if "error" not in stream_results:
            ...
            if stream_results["first_chunk_time"]:
                assert (
                    stream_results["first_chunk_time"] < 5.0
    ...
    def test_model_parameter_compatibility(self, openai_client):
        ...
        for model_name in model_names:
            try:
                ...
                assert (
                    response["model"] == model_name or "test-model" in response["model"]
                )
    ...
    def test_message_roles_compatibility(self, openai_client):
        ...
        try:
            response = openai_client.create_chat_completion(
                model="test-model", messages=messages, max_tokens=50
            )
