...
class TestNginxRouting:
    ...
    async def test_public_api_routing(self, nginx_client):
        ...
        if health_result:  # Health endpoint might not exist
            assert (
                health_result["expects_auth"] == False
                or health_result["unauthenticated"]["status_code"] == 200
            )
    ...
    async def test_internal_api_routing(self, nginx_client):
        ...
        # Internal data routes should require authentication; registration is public.
        for path, result in results.items():
            if result["expects_auth"]:
                assert (
                    result["requires_auth"] == True
                ), f"Internal route {path} should require authentication"
                assert (
                    result["unauthenticated"]["status_code"] == 401
                ), f"Internal route {path} should return 401 without auth"
            else:
                assert (
                    result["requires_auth"] == False
                ), f"Internal route {path} should be publicly reachable"
    ...
    async def test_frontend_routing(self, nginx_client):
        ...
        assert root_result["status_code"] in [
            200,
            404,
        ]  # 404 is acceptable if Next.js not running
        ...
        for path, result in results.items():
            if path != "/":  # Root might have different behavior
                assert result["status_code"] in [
                    ...
                    404,
                    500,
                ], f"Frontend route {path} returned unexpected status {result['status_code']}"
    ...
    async def test_cors_headers(self, nginx_client):
        ...
        assert (
            len(cors_headers) > 0 or request.get("status_code") == 401
        )  # Auth might block before CORS
    ...
    async def test_websocket_support(self, nginx_client):
        ...
        assert ws_result["status_code"] in [
            101,
            200,
            404,
            426,
        ], f"Unexpected WebSocket response: {ws_result['status_code']}"

        # If upgrade attempted, check headers
        if ws_result["upgrade_attempted"]:
            assert (
                "upgrade" in ws_result.get("upgrade_header", "").lower()
                or "websocket" in ws_result.get("connection_header", "").lower()
            )
    ...
    async def test_health_endpoints(self, nginx_client):
        ...
        healthy_endpoints = [
            endpoint for endpoint, result in health_results.items() if result["healthy"]
        ]
        assert (
            len(healthy_endpoints) > 0
        ), "No health endpoints are responding correctly"
    ...
    async def test_static_file_caching(self, nginx_client):
        ...
        for file_path, result in static_results.items():
            if result["status_code"] == 200:  # Only check if file was served
                assert result[
                    "cached"
                ], f"Static file {file_path} should have cache headers"
    ...
    async def test_error_handling(self, nginx_client):
        ...
        for path, result in error_results.items():
            ...
            assert (
                result["actual_status"] >= 400
            ), f"Error path {path} should return error status"
            ...
            if result["expected_status"] == 404:
                assert result["actual_status"] in [
                    404,
                    500,
                ], f"404 path {path} returned {result['actual_status']}"
    ...
    async def test_request_routing_headers(self, http_session):
        ...
        async with http_session.get(
            f"{self.BASE_URL}/api/v1/models", headers=headers_to_test
        ) as response:
            ...
            assert response.status in [
                ...
                200,
                422,
            ], f"Unexpected status for header test: {response.status}"
    ...
    async def test_request_size_limits(self, http_session):
        ...
        async with http_session.post(
            f"{self.BASE_URL}/api/v1/chat/completions", json=large_payload
        ) as response:
            ...
            assert response.status in [
                ...
                400,
                422,
            ], f"Large request returned {response.status}"

    @pytest.mark.asyncio
    async def test_concurrent_requests(self, nginx_client):
        """Test nginx handling of concurrent requests"""
        load_results = await nginx_client.test_load_balancing(
            20
        )  # 20 concurrent requests

        assert load_results["total_requests"] == 20
        assert (
            load_results["failure_rate"] < 0.5
        ), f"High failure rate: {load_results['failure_rate']}"
        assert (
            load_results["average_response_time"] < 5.0
        ), f"Slow response time: {load_results['average_response_time']}s"
    ...
    def _is_feature_working(self, feature_name: str, results: Dict[str, Any]) -> bool:
        ...
        if feature_name == "health_endpoints":
            ...
            return (
                preflight.get("status_code") in [204, 200]
                or len(preflight.get("cors_headers", {})) > 0
            )
