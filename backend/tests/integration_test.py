...
class LiveModuleIntegrationTest:
    ...
    async def test_all_modules_loaded(self):
        ...
        for expected in expected_modules:
            assert (
                expected in loaded_modules
            ), f"Module {expected} not found in {loaded_modules}"
    ...
    async def test_module_stats_availability(self, modules_data: Dict[str, Any]):
        ...
        for module in modules_data["modules"]:
            ...
            if "stats" in module:
                ...
                print(
                    f"✓ Module '{module_name}' provides stats: {len(module['stats'])} metrics"
                )
            else:
        ...
        assert (
            len(stats_modules) >= 3
        ), f"Expected at least 3 modules with stats, got {len(stats_modules)}"
    ...
    async def test_specific_module_functionality(self, modules_data: Dict[str, Any]):
        ...
        if "security" in modules_by_name:
            ...
            assert security_mod.get(
                "initialized", False
            ), "Security module should be initialized"
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                for module in data["modules"]:
                    ...
                    print(
                        f"   {status} {module['name']} v{module['version']} ({stats_count} stats)"
                    )
