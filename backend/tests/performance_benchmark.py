...
class PerformanceBenchmark:
    ...
    async def measure_concurrent_performance(
        ...
        print(
            f"🔥 Benchmarking concurrent performance ({concurrent_users} users, {requests_per_user} requests each)..."
        )
    ...
    async def monitor_resources_during_test(
    ...
    def generate_performance_report(self) -> Dict[str, Any]:
        ...
        report = {
            ...
            "system_resources": {
                ...
                "avg_cpu": (
                    statistics.mean([m.cpu_percent for m in self.resource_metrics])
                    if self.resource_metrics
                    else 0
                ),
                "avg_memory": (
                    statistics.mean([m.memory_percent for m in self.resource_metrics])
                    if self.resource_metrics
                    else 0
                ),
                "peak_cpu": (
                    max([m.cpu_percent for m in self.resource_metrics])
                    if self.resource_metrics
                    else 0
                ),
                "peak_memory": (
                    max([m.memory_percent for m in self.resource_metrics])
                    if self.resource_metrics
                    else 0
                ),
            },
    ...
    def generate_recommendations(self) -> List[str]:
        ...
        if "api_performance" in self.results:
            ...
            if api_metrics.avg_time > 1.0:
                recommendations.append(
                    "⚠️ API response time is slow (>1s). Consider optimizing database queries and caching."
                )

            if api_metrics.success_rate < 0.95:
                recommendations.append(
                    "⚠️ API success rate is low (<95%). Check for connection timeouts and error handling."
                )

            if api_metrics.throughput < 10:
                recommendations.append(
                    "⚠️ Low API throughput (<10 req/s). Consider connection pooling and async optimizations."
                )
        ...
        if "concurrent_performance" in self.results:
            ...
            if concurrent_metrics.avg_time > api_metrics.avg_time * 2:
                recommendations.append(
                    "⚠️ Performance degrades under load. Consider horizontal scaling or caching."
                )
        ...
        if self.resource_metrics:
            ...
            if avg_cpu > 80:
                recommendations.append(
                    "⚠️ High CPU usage detected. Consider optimizing computational tasks."
                )

            if avg_memory > 80:
                recommendations.append(
                    "⚠️ High memory usage detected. Check for memory leaks and optimize data structures."
                )
        ...
        if not recommendations:
            ...
            recommendations.append(
                "💡 Consider implementing Redis caching for further performance gains."
            )
            recommendations.append(
                "💡 Monitor performance regularly as the system scales."
            )
    ...
    def print_results(self, report: Dict[str, Any]):
        ...
        if "performance_metrics" in report:
            ...
            for test_name, metrics in report["performance_metrics"].items():
                ...
                print(
                    f"    Min/Max Time: {metrics['min_time']:.3f}s / {metrics['max_time']:.3f}s"
                )
