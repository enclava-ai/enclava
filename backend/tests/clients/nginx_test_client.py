...
class NginxTestClient:
    ...
    async def test_public_api_routes(self) -> Dict[str, Any]:
        ...
        for route in routes_to_test:
            ...
            results[route["path"]] = {
                ...
                "auth_working": (
                    (
                        result_unauth["status_code"] == 401
                        and result_auth["status_code"] != 401
                    )
                    if route["expected_auth"]
                    else True
                ),
            }
    ...
    async def test_internal_api_routes(self) -> Dict[str, Any]:
        ...
        routes_to_test = [
            {
                "path": "/api-internal/v1/auth/me",
                "method": "GET",
                "expected_auth": True,
            },
            {
                "path": "/api-internal/v1/auth/register",
                "method": "POST",
                "expected_auth": False,
            },
            {
                "path": "/api-internal/v1/tool-calling/agent/configs",
                "method": "GET",
                "expected_auth": True,
            },
            {
                "path": "/api-internal/v1/rag/collections",
                "method": "GET",
                "expected_auth": True,
            },
        ]
        ...
        for route in routes_to_test:
            ...
            results[route["path"]] = {
                ...
                "auth_working": result_unauth["status_code"] == 401
                and result_auth["status_code"] != 401,
            }
    ...
    async def test_frontend_routes(self) -> Dict[str, Any]:
        ...
        routes_to_test = ["/", "/dashboard", "/agents", "/rag", "/settings", "/login"]
    ...
    async def test_cors_headers(self) -> Dict[str, Any]:
        ...
        preflight_result = await self.test_route(
            "/api/v1/models", "OPTIONS", cors_headers
        )
        cors_tests["preflight"] = {
            "status_code": preflight_result["status_code"],
            "cors_headers": {
                k: v
                for k, v in preflight_result["headers"].items()
        ...
        cors_tests["request"] = {
            ...
            "cors_headers": {
                k: v
                for k, v in cors_result["headers"].items()
                if k.lower().startswith("access-control")
            },
        }
    ...
    async def test_websocket_support(self) -> Dict[str, Any]:
        ...
        return {
            "status_code": result["status_code"],
            "upgrade_attempted": result["status_code"]
            in [101, 426],  # 101 = Switching Protocols, 426 = Upgrade Required
    ...
    async def test_health_endpoints(self) -> Dict[str, Any]:
        ...
        health_endpoints = ["/health", "/api/v1/health", "/test-status"]
    ...
    async def test_static_file_handling(self) -> Dict[str, Any]:
        ...
        static_files = ["/_next/static/test.js", "/favicon.ico", "/static/test.css"]
        ...
        for file_path in static_files:
            ...
            results[file_path] = {
                ...
                "cached": "cache-control" in result["headers"]
                or "expires" in result["headers"],
            }
