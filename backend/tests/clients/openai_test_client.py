...
class OpenAITestClient:
    ...
    def create_embedding(self, model: str, input_text: str, **kwargs) -> Dict[str, Any]:
    ...
    def test_error_handling(self) -> Dict[str, Any]:
        ...
        try:
            no_key_client = OpenAI(base_url=self.base_url, api_key="missing-key")
        ...
        try:
            ...
            test_cases.append(
                {"test": "rate_limiting", "result": "no_rate_limit_triggered"}
            )
...
class AsyncOpenAITestClient:
    ...
    async def test_streaming_performance(self) -> Dict[str, Any]:
        ...
        payload = {
            "model": "test-model",
            "messages": [
                {"role": "user", "content": "Generate a long response about AI"}
            ],
            "stream": True,
            "max_tokens": 500,
        }
        ...
        return {
            ...
            "avg_chunk_interval": (
                sum(chunk_times) / len(chunk_times) if chunk_times else 0
            ),
...
def validate_openai_response_format(
    ...
    if endpoint_type == "chat_completion":
        ...
        models = response if isinstance(response, list) else [response]
        for model in models:
