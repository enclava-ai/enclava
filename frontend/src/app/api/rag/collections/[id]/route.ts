  ...
  try {
    ...
    if (!backendResponse.ok) {
      ...
      return NextResponse.json(
        { success: false, error: errorData.detail || errorData.error || 'Failed to delete collection' },
...
export async function GET(
  ...
  try {
    ...
    if (!backendResponse.ok) {
      const errorData = await backendResponse.json().catch(() => ({ error: 'Unknown error' }))
      return NextResponse.json(
        { success: false, error: errorData.detail || errorData.error || 'Failed to fetch collection' },
        { status: backendResponse.status }
      )
    }
